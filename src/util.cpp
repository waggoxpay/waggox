    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                if(ptrCategory->count(std::string("waggoxcoin"))) {
...
#else
    const char* pszModule = "WAGGOXCOIN2";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "WAGGOXCOIN2";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/WAGGOXCOIN2";
#else
    // Unix
    return pathRet / ".WAGGOXCOIN2";
#endif
#endif
}
